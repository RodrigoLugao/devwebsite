...
const useRecuperarPecasPorSlugCategoriaComPaginacao = (
  ...
  > => {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Erro ao buscar peças: ${response.status} - ${errorText || "Resposta vazia"}`
      );
    }

    return (await response.json()) as Page<Peca>;
  };

  return useQuery<Page<Peca>, Error>({ 
    queryKey: ["pecas-por=slug", slugCategoria || "todas", pageable],
